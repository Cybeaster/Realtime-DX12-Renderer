                                    ...
                                    const D3D12_HEAP_PROPERTIES& heapProps)
{
	...
	THROW_IF_FAILED(m_device->CreateCommittedResource(&heapProps, D3D12_HEAP_FLAG_NONE, &bufDesc, initState, nullptr, IID_PPV_ARGS(&pBuffer)));
	return pBuffer;
}
...
IDxcBlob* CompileShaderLibrary(LPCWSTR fileName)
{
	...
	if (!pCompiler)
	{
		THROW_IF_FAILED(DxcCreateInstance(CLSID_DxcCompiler, __uuidof(IDxcCompiler), (void**)&pCompiler));
		THROW_IF_FAILED(DxcCreateInstance(CLSID_DxcLibrary, __uuidof(IDxcLibrary), (void**)&pLibrary));
		THROW_IF_FAILED(pLibrary->CreateIncludeHandler(&dxcIncludeHandler));
	}
	...
	THROW_IF_FAILED(pLibrary->CreateBlobWithEncodingFromPinned(
	    (LPBYTE)sShader.c_str(), (uint32_t)sShader.size(), 0, &pTextBlob));

	// Compile
	IDxcOperationResult* pResult;
	THROW_IF_FAILED(pCompiler->Compile(pTextBlob, fileName, L"", L"lib_6_3", nullptr, 0, nullptr, 0, dxcIncludeHandler, &pResult));

	// Verify the result
	HRESULT resultCode;
	THROW_IF_FAILED(pResult->GetStatus(&resultCode));
	...
	IDxcBlob* pBlob;
	THROW_IF_FAILED(pResult->GetResult(&pBlob));
	return pBlob;
}
                                           ...
                                           D3D12_DESCRIPTOR_HEAP_TYPE type, bool shaderVisible)
{
	...
	THROW_IF_FAILED(device->CreateDescriptorHeap(&desc, IID_PPV_ARGS(&pHeap)));
	return pHeap;
}
                          ...
                          std::vector<UINT>& outputIndices)
{
	struct Cube
	{
		Cube(const DirectX::XMVECTOR& tlf, float s)
		    : m_topLeftFront(tlf), m_size(s)
		{
		}
		                 ...
		                 const DirectX::XMVECTOR& bottomLeft4, const DirectX::XMVECTOR& dx, const DirectX::XMVECTOR& dy, bool flip)
		{
			...
			if (flip)
			{
				normal = DirectX::XMVectorMultiply(normal, DirectX::XMVectorReplicate(-1.f));
			...
			vertices.push_back({ { bottomLeft.x + dx.m128_f32[0], bottomLeft.y + dx.m128_f32[1], bottomLeft.z + dx.m128_f32[2], 1.f },
			                     n,
			                     { 0.5f, 1.f, 0.f, 1.f } });
			vertices.push_back({ { bottomLeft.x + dy.m128_f32[0], bottomLeft.y + dy.m128_f32[1], bottomLeft.z + dy.m128_f32[2], 1.f },
			                     n,
			                     { 0.5f, 0.f, 1.f, 1.f } });

			vertices.push_back({ { bottomLeft.x + dx.m128_f32[0] + dy.m128_f32[0],
			                       bottomLeft.y + dx.m128_f32[1] + dy.m128_f32[1],
			                       bottomLeft.z + dx.m128_f32[2] + dy.m128_f32[2],
			                       1.f },
			                     n,
			                     { 0.f, 1.f, 0.f, 1.f } });
		}
